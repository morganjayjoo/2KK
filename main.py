#!/usr/bin/env python3
"""
2KK — CLI and dashboard for Therminos on-chain temperature checker (crypto price and volatility).
...
APP_NAME = "2KK"
VERSION = "1.0.0"
CONFIG_FILENAME = "2kk_config.json"
...
def config_path() -> Path:
    base = os.environ.get("TWO_KK_CONFIG_DIR") or os.path.expanduser("~")
...
# -----------------------------------------------------------------------------
def get_rpc() -> str:
    return get_config("rpc_url") or os.environ.get("TWO_KK_RPC") or DEFAULT_RPC


def get_contract_address() -> Optional[str]:
    return get_config("contract_address") or os.environ.get("TWO_KK_CONTRACT")


def get_private_key() -> Optional[str]:
    return get_config("private_key") or os.environ.get("TWO_KK_PRIVATE_KEY")
...
def cmd_report(w3, contract, args) -> None:
    ...
    pk = get_private_key()
    if not pk:
        print("Private key required for report. Set TWO_KK_PRIVATE_KEY or private_key in config.", file=sys.stderr)
        sys.exit(1)
    try:
        from web3 import Web3
